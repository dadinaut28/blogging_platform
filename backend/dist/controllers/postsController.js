...
export async function postPosts(req, res) {
    try {
        const { title, subTitle, content, readingTime, published, categoryId } = req.body;
...
export async function getPostsResponses(req, res) {
    try {
...
export async function postPostsResponses(req, res) {
    try {
        let userId;
        if (typeof req.userInfo !== "string") {
            userId = req.userInfo?.userId;
...
export async function patchPostResponse(req, res) {
    try {
        ...
        if (userId !== response?.user.id) {
            return res.status(401).json({
                message: "Access denied: user attempting to modify reponse is not the owner !",
            });
        }
...
export async function delePostResponse(req, res) {
    try {
        const { responseId } = req.params;
        let userId;
        if (typeof req.userInfo !== "string") {
        ...
        if (userId !== response?.user.id) {
            return res.status(401).json({
                message: "Access denied: user attempting to delete reponse is not the owner !",
            });
        }
